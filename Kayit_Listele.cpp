...
TabKayitListele::TabKayitListele(QWidget *parent) : QWidget(parent) {
    ...
    QPushButton *deleteLogBtn = new QPushButton("Log Kayıtlarını Sil");
...
void TabKayitListele::guncellePenceresiAc(QTableWidgetItem *item) {
    ...
    connect(updateBtn, &QPushButton::clicked, [=]() {
        ...
        if(updateQuery.exec()) {
            ...
            QMessageBox::warning(dialog, "Hata", "Güncelleme sırasında hata oluştu\nTest esnasında güncelleme yapmayiniz", QMessageBox::Ok);
        }
    });
...
void TabKayitListele::verileriGoster() {
    ...
    if (query2.exec()) {
        while (query2.next()) {
            allLogs += query2.value(0).toString() + "\n";
        }
    }
    if (allLogs.isEmpty()) {
        logViewer->setPlainText("Log bulunamadı.");
    } else {
        logViewer->setPlainText(allLogs);
    }
}
