...
TabTestBaslat::TabTestBaslat(QWidget *parent) : QWidget(parent) {
    ...
    minRssiCombo->setCurrentText("-10");
...
void TabTestBaslat::seriVeriOku() {
    ...
    while (readBuffer.contains('\n')) {
        ...
        if (text.contains("Link-layer address:", Qt::CaseInsensitive)) {
            ...
            insert.exec();
            logText = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss");
            QSqlQuery log;
            log.prepare("INSERT INTO modem_loglari (seri_no, log) VALUES (:seri_no, :log)");
            log.bindValue(":seri_no", cleanedSerial);
            log.bindValue(":log","Restart Algılandı Son Test Tarihi: " + logText);
            log.exec();
        }
    ...
    if (!switchedTo9600) {
        ...
        if (data.startsWith("/?") && data.endsWith("!\r\n") && data.length() > 5) {
            ...
            if (findMeterBySerialNumber(serialNumber)) {
                ...
                activeMeterID = 0; // activeMeterID'yi sıfırla genel sorgu ACK ile karışmasın
            } else {
        ...
        //ADRESLİ SORGU İÇİN ACK KONTROLÜ
